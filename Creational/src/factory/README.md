...
## Example 5: Payment Gateway Client Factory

**Focus:** Integrating Business Logic (Routing) into the creation process.
...
public class PaymentMain {
    public static void main(String[] args) {
        ...
        context.amount = 500;
        context.currency = Currency.getInstance("INR");
        context.countryCode = "IN";

        // Client doesn't know which gateway is picked
        PaymentGateway gateway = factory.selectGateway(context);
        gateway.processPayment(context.amount, context.currency);
    }
}
```